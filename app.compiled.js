...
function archiveTournament(tName, format, teams, matches) {
  if (!_fbdb) return;
  const record = {
    id: _SESSION + '_' + Date.now(),
    name: tName || 'Untitled',
    format,
    teams,
    matches,
  ...
  _fbdb.collection('flipbad-history').add(record).catch(console.error);
}
...
const Sty = {
  ...
  bottomBar: {
    ...
    background: "linear-gradient(to top, #06040c 85%, transparent)"
  },
  card: {
    background: P.bg1,
...
function App() {
  ...
  useEffect(() => {
    function onRemote(e) {
      const d = e.detail;
      // Only apply remote update if we haven't touched that key locally this session
      if ('fb-registry' in d && !_touched.has('fb-registry')) setRegistry(d['fb-registry']);
      if ('fb-tname' in d && !_touched.has('fb-tname')) setTName(d['fb-tname']);
      if ('fb-format' in d && !_touched.has('fb-format')) setFormat(d['fb-format']);
      if ('fb-teams' in d && !_touched.has('fb-teams')) setTeams(d['fb-teams']);
      if ('fb-matches' in d && !_touched.has('fb-matches')) setMatches(d['fb-matches']);
      if ('fb-tinputs' in d && !_touched.has('fb-tinputs')) setTeamInputs(d['fb-tinputs']);
    }
  ...
  useEffect(() => {
    if (!_fbdb) return;
    const unsub = _fbdb.collection('flipbad-history')
  ...
  function finalizeMatch(id) {
    setMatches(prev => {
      const updated = prev.map(m => {
        if (m.id !== id) return m;
        let s1 = 0, s2 = 0;
        m.sets.forEach(s => { if (s.t1 > s.t2) s1++; else if (s.t2 > s.t1) s2++; });
        return {...m, status: "completed", winner: s1 > s2 ? m.team1 : s2 > s1 ? m.team2 : null};
      });
      saveNow('fb-matches', updated);
      // Archive when all matches complete
      if (updated.every(m => m.status === 'completed')) {
        archiveTournament(tName, format, teams, updated);
      }
      return updated;
    });
    setMatchId(null);
...
function HistoryScreen({ history, onBack }) {
  return /*#__PURE__*/React.createElement("div", {style: Sty.page},
    /*#__PURE__*/React.createElement("div", {style: Sty.wrap},
      /*#__PURE__*/React.createElement(TopBar, {title: "Tournament History", sub: "Past tournaments", onBack: onBack}),
      history.length === 0
        ? /*#__PURE__*/React.createElement("div", {style:{textAlign:"center",padding:"60px 20px",color:"#9f7aea",fontSize:14}},
            "🏅 No completed tournaments yet"
          )
        : history.map(t => {
            const wins = {};
            t.teams && t.teams.forEach(tm => { wins[tm.name] = 0; });
            t.matches && t.matches.forEach(m => { if (m.winner) wins[m.winner] = (wins[m.winner]||0) + 1; });
            const sorted = Object.entries(wins).sort((a,b)=>b[1]-a[1]);
            const date = t.completedAt ? new Date(t.completedAt).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"}) : "";
            return /*#__PURE__*/React.createElement("div", {
              key: t.id,
              style: {background:P.bg1,border:`1px solid ${P.border}`,borderRadius:14,padding:"14px 16px",marginBottom:10}
            },
              /*#__PURE__*/React.createElement("div", {style:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}},
                /*#__PURE__*/React.createElement("div", null,
                  /*#__PURE__*/React.createElement("div", {style:{color:P.white,fontWeight:800,fontSize:15}}, t.name || "Untitled"),
                  /*#__PURE__*/React.createElement("div", {style:{color:"#9f7aea",fontSize:11,marginTop:2}}, t.format, " · ", t.matches ? t.matches.length : 0, " matches")
                ),
                /*#__PURE__*/React.createElement("div", {style:{color:"#9f7aea",fontSize:10,textAlign:"right"}}, date)
              ),
              /*#__PURE__*/React.createElement("div", {style:{borderTop:`1px solid ${P.borderB}`,paddingTop:8}},
                sorted.slice(0,3).map(([name,w],i) =>
                  /*#__PURE__*/React.createElement("div", {key:name,style:{display:"flex",alignItems:"center",gap:8,padding:"3px 0"}},
                    /*#__PURE__*/React.createElement("span", {style:{fontSize:13}}, i===0?"🥇":i===1?"🥈":"🥉"),
                    /*#__PURE__*/React.createElement("span", {style:{color:P.white,fontSize:13,flex:1}}, name),
                    /*#__PURE__*/React.createElement("span", {style:{color:"#a855f7",fontSize:12,fontWeight:700}}, w, " W")
                  )
                )
              )
            );
...
function HomeScreen({
  ...
  return /*#__PURE__*/React.createElement("div", {
    style: {
      ...
      padding: "clamp(12px,3vw,28px) 20px clamp(16px,3vw,28px)",
      position: "relative"
    }
  ...
  })), /*#__PURE__*/React.createElement("div", {style:{display:"flex",gap:16,marginTop:22,alignItems:"center"}},
    /*#__PURE__*/React.createElement("button", {
      onClick: onPlayers,
      style:{background:"transparent",border:"none",color:"#c084fc",fontSize:11,cursor:"pointer",fontFamily:F,letterSpacing:".04em",display:"flex",alignItems:"center",gap:5}
    }, "👤 Players"),
    /*#__PURE__*/React.createElement("span", {style:{color:"rgba(168,85,247,.3)"}}, "|"),
    /*#__PURE__*/React.createElement("button", {
      onClick: onHistory,
      style:{background:"transparent",border:"none",color:"#c084fc",fontSize:11,cursor:"pointer",fontFamily:F,letterSpacing:".04em",display:"flex",alignItems:"center",gap:5}
    }, "🏅 History")
...
function TournamentSetup({
  ...
  }, "+ Add Team"))), /*#__PURE__*/React.createElement("div", {style: Sty.bottomBar},
    /*#__PURE__*/React.createElement("button", {
      style: {
        ...Sty.heroBtn,
...
function MatchScorer({
  ...
  }, "Reset Match")) : null), m.status !== "completed" && /*#__PURE__*/React.createElement("div", {style: Sty.bottomBar},
    /*#__PURE__*/React.createElement("button", {
      style: Sty.heroBtn,
